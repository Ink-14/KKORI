...
impl RuleCheckerBuilder {
    ...
    pub fn add_rule(
        ...
        self.nodes[current].output_message = Some(message);
...
#[pymethods]
impl RuleChecker {
    pub fn check(&self, _py: Python, tokens: &Bound<'_, PyList>) -> PyResult<Vec<SpellError>> {
        let enriched = self.enrich_tokens(tokens)?;
        Ok(self.check_inner(enriched))
    }
}
...
impl RuleChecker {
    fn enrich_tokens(&self, tokens: &Bound<'_, PyList>) -> PyResult<Vec<EnrichedToken>> {
        tokens.iter()
            .map(|t| {
                ...
                let len:   u8     = t.getattr("len")?.extract()?;
    ...
    fn check_inner(&self, tokens: Vec<EnrichedToken>) -> Vec<SpellError> {
