...
from src.engines.configs.rule_constants import 모음연결어미_FORMS, ㄹ사용불가_연결어미_FORMS, JOSA_TARGETS, 피우다_TARGETS, 펴다_TARGETS, 색상_ADJ_FORMS
...
_ADD = [
    ...
    *rule().id("ADD_설레다카메라에 매우 인상적으로 담겨지기도 했었다")
    .tag_form(Tag.동사, "설레이")
    .any()
    ...
    *rule().id("ADD_담기다")
    ...
    .tag_form(Tag.보조용언, "지")
    .any()
    .msg("'담겨지다'는 이중 피동 표현이므로 'merge((\"담\", \"동사\"), (\"어\", \"연결어미\"), (\"지\", \"연결어미\"), ({dform[3]}, {dtag[3]}))'batchim(\"으로\", \"로\") 쓸 것을 권장합니다.").build(),
...
_REP_VERBS = [
    ...
    *rule().id("REP_띠다")
    .AND(tag(Tag.일반명사), forms({"색", "빛", "빛깔", "분홍색", "분홍빛", "빨간색", "빨간빛", "붉은색", "붉은빛", "파란빛", "푸른빛", "파란색", "푸른색", "노란색", "노란빛", "초록빛", "초록색", "검은색", "검정색", "검은빛", "검정빛", "핏빛", "하얀색", "하얀빛", "주황색", "주홍빛", "다홍색", "다홍빛", "갈색", "갈색빛", "하늘색", "하늘빛", "금색", "금빛", "보라색", "보랏빛", "성격", "색채", "형태", "모양", "활기", "성질", "성향", "분위기", "관련", "폭력성", "성정", "살기", "홍조", "광기", "미소", "모습", "형체", "하락세", "상승세"})).context()
    ...
    *rule().id("REP_맞히다_1")
    .AND(tag(Tag.일반명사), forms({"정답", "답", "문제", "퀴즈", "암호", "과녁"})).context()
    .any().opt().context()
    .any().opt().context()
    .tag_form(Tag.동사, "맞추")
    ...
    *rule().id("REP_제치다")
    ...
    .AND(tag(Tag.동사), forms({"제끼", "재끼"}))
    .any()
    .msg("'merge(({dform[0]}, {dtag[0]}), (\"어\", \"연결어미\"))merge((\"젖히\", \"동사\"), ({dform[3]}, {dtag[3]}))'batchim(\"이\", \"가\") 올바른 표현입니다.").build(),
...
_REP_NNG = [
    ...
    *rule().id("REP_율_ㄴ받침")
    ...
    .msg("'{dform[0]}율'의 오타가 아닌가요? ㄴ받침으로 끝나는 명사에는 '율'을 사용해야 합니다.").build(),
    
    *rule().id("REP_율_받침없음")
    .no_batchim()
    .tag_form(Tag.명사파생접미사, "률")
    .msg("'{dform[0]}율'의 오타가 아닌가요? 받침 없는 명사에는 '율'을 사용해야 합니다.").build(),
    
    *rule().id("REP_률")
    .AND(any_batchim(), NOT(batchim("ᆫ")))
    .tag_form(Tag.명사파생접미사, "율")
    .msg("'{dform[0]}률'의 오타가 아닌가요? ㄴ받침 이외의 받침 있는 명사에는 '률'을 사용해야 합니다.").build(),
