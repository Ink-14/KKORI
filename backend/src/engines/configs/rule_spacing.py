...
GENERAL_SPACING_ERRORS: list[KoSpellRules] = [
    ...
    *rule()
    ...
    .msg("'안 merge(({dform[1]}, \"형용사\"), (\"다\", \"종결어미\"))'로 띄어 써야 합니다.").build(),
    
    *rule().id("GENERAL_안_동사_띄어쓰기")
    .tag_form(Tag.일반부사, "안")
    .AND(NOT(form("되")), tags({Tag.동사, Tag.동사불규칙활용, Tag.동사규칙활용})).if_not_spaced()
...
_SPACING_ERRORS = [
    ...
    *rule().id("VV_지 못하다 이외_못 하다_띄어쓰기")
    ...
    .AND(
        AND(
            AND(
                NOT(AND(tag(Tag.연결어미), forms({"지", "도"}))),
                NOT(tag_form(Tag.목적격조사, "ᆯ"))
            ),
            NOT(tags({Tag.일반부사, Tag.일반명사}))
        ),
        NOT(AND(tag(Tag.보조사), forms({"ᆫ", "도"})))
    ).context()
    .tag_form(Tag.일반부사, "못")
    .tag_form(Tag.동사, "하").if_not_spaced()
...
_NNB = [
    *rule().id("NNB_체언뒤_띄어쓰기")
    .tags({Tag.일반명사, Tag.의존명사, Tag.명사파생접미사, Tag.의존명사, Tag.명사형전성어미, Tag.대명사})
    .tag(Tag.닫는부호).opt()
    .AND(tag(Tag.의존명사), forms({"쪽", "측", "때문", "및", "따위", "시", "무렵"})).if_not_spaced()
    .msg("'{dform[0]} {form[0]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),
    
    *rule().id("NNB_몇_띄어쓰기")
    .tag_form(Tag.관형사, "몇")
    .AND(tag(Tag.의존명사), forms({"번", "개", "명", "년"})).if_not_spaced()
    .msg("'몇 {form[1]}'batchim(\"으로\",\"로\") 띄어 써야 합니다.").build(),
    
    *rule().id("NNB_관형사형전성어미뒤_띄어쓰기")
    .AND(tag(Tag.관형사형전성어미), forms({"ᆫ", "는", "은"})).context()
    .AND(tag(Tag.의존명사), forms({"격", "자", "편", "양"})).if_not_spaced()
    ...
    *rule().id("NNB_지_MM_띄어쓰기")
    .AND(tag(Tag.관형사형전성어미), forms({"ᆫ", "은"})).context()
    .tag_form(Tag.의존명사, "지").if_not_spaced()
    .any().opt().context()
    ...
    *rule().id("NNB_숫자_뒤_단위_1_띄어쓰기")
    ...
    .AND(tag(Tag.의존명사), NOT(forms({"가지", "월"}))).if_not_spaced()
    ...
    *rule().id("NNB_숫자_뒤_단위_2_띄어쓰기")
    ...
    .msg("'{dform[0]}{dform[1]} {dform[2]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_것_1_띄어쓰기")
    .tags({Tag.관형사형전성어미, Tag.관형격조사}).context()
    .AND(tag(Tag.의존명사), forms({"것"})).if_not_spaced()
    .NOT(tag_form(Tag.주격조사, "이")).context()
    .tags(TagGroup.용언 | {Tag.긍정지정사, Tag.부정지정사}).context()
    .msg("'것'을 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNB_것_2_띄어쓰기")
    .AND(tags({Tag.관형사}), forms({"그딴", "그런", "다른", "모든", "이런", "그런"})).context()
    .AND(tag(Tag.의존명사), forms({"것"})).if_not_spaced()
    ...
    *rule().id("NNB_것_6_띄어쓰기")
    ...
    .tag_form(Tag.의존명사, "것").if_not_spaced()
    .tag(Tag.긍정지정사).context()
    .AND(tags({Tag.연결어미, Tag.종결어미}), NOT(forms({"라거나"}))).context()
    .msg("'것'을 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNB_걸_동사집합_띄어쓰기")
    .tag_form(Tag.의존명사, "거").if_not_spaced()
    .tag_form(Tag.목적격조사, "ᆯ")
    .AND(tag(Tag.동사), forms({"좋아하", "도와주"})).context()
    ...
    *rule().id("NNB_걸_1_1_띄어쓰기")
    .AND(tag(Tag.관형사), forms({"그런", "딴", "모든", "어떤", "이런", "한"})).context()
    ...
    *rule().id("NNB_걸_3_띄어쓰기")
    .AND(tag(Tag.관형사형전성어미), forms({"다는", "단", "란", "라는"})).context()
    .tag_form(Tag.의존명사, "거").if_not_spaced()
    .tag_form(Tag.목적격조사, "ᆯ")
    ...
    *rule().id("NNB_게_1_띄어쓰기")
    ...
    .tag_form(Tag.의존명사, "것").if_not_spaced()
    .tag_form(Tag.주격조사, "이")
    .AND(tag(Tag.형용사), forms({"있", "어딨", "좋", "낫", "없"})).context()
    .msg("'것/게'를 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_게_3_띄어쓰기")
    .AND(tag(Tag.관형사형전성어미), forms({"라는", "ᆫ", "는", "ᆫ다는", "다는"})).context()
    .tag_form(Tag.의존명사, "것").if_not_spaced()
    .tag_form(Tag.주격조사, "이")
    .msg("'것/게'를 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_만큼_띄어쓰기")
    .tags({Tag.관형사형전성어미, Tag.관형격조사}).context()
    .AND(tag(Tag.의존명사), forms({"만큼"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\",\"를\") 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_데_동사_띄어쓰기")
    .tag(Tag.관형사형전성어미).context()
    .tag_form(Tag.의존명사, "데").if_not_spaced()
    .AND(tag(Tag.동사), forms({"비하", "반하", "들", "더하"})).context()
    .msg("'데'를 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_데_형용사_띄어쓰기")
    ...
    .AND(tag(Tag.형용사), forms({"좋"})).context()
    .msg("'데'를 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNB_데_명사_띄어쓰기")
    .tag_form(Tag.의존명사, "데").if_not_spaced()
    .tag(Tag.일반부사).opt().context()
    .AND(tag(Tag.일반명사), forms({"필요", "효과", "혈안", "도움"})).context()
    .msg("'데'를 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_데_결정적 도움을 주다_띄어쓰기")
    .tag_form(Tag.의존명사, "데").if_not_spaced()
    .tag_form(Tag.일반명사, "결정").context()
    .tag_form(Tag.명사파생접미사, "적").context()
    .tag(Tag.긍정지정사).opt().context()
    .tag_form(Tag.관형사형전성어미, "ᆫ").opt().context()
    ...
    *rule().id("NNB_한 번_띄어쓰기")
    ...
    .tag_form(Tag.관형사, "한")
    .tag_form(Tag.의존명사, "번").if_not_spaced()
    .AND(tags(TagGroup.조사), NOT(form("은"))).if_not_spaced().context()
    .msg("'한 번'으로 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNB_관형사_때문_띄어쓰기")
    .AND(tag(Tag.관형사), forms({"그", "이", "저"})).context()
    .tag_form(Tag.의존명사, "때문").if_not_spaced()
    .msg("'{form[0]} 때문'으로 띄어 써야 합니다.").build(),

    *rule().id("NNB_등_띄어쓰기")
    .AND(tags({Tag.일반명사, Tag.의존명사, Tag.명사파생접미사, Tag.의존명사, Tag.명사형전성어미, Tag.대명사}), NOT(tag_form(Tag.의존명사, "등")))
    .tag(Tag.닫는부호).opt()
...
_NNG = [
    *rule().id("NNG_체언_명사_띄어쓰기")
    .tags({Tag.일반명사, Tag.대명사, Tag.관형격조사, Tag.관형사형전성어미}).context()
    .AND(tag(Tag.일반명사), forms({"편", "정도", "말", "때", "경우"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\",\"를\") 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNG_일반명사_띄어쓰기")
    .tag(Tag.일반명사).context()
    .AND(tag(Tag.일반명사), forms({"고증", "심의", "겨냥"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\",\"를\") 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNG_일반명사/의존명사_띄어쓰기")
    ...
    *rule().id("NNG_관형사형전성어미 뒤_띄어쓰기")
    .tag(Tag.관형사형전성어미).context()
    .AND(tag(Tag.일반명사), forms({"모양"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\", \"를\") 앞 말과 띄어 써야 합니다.").build(),
    
    *rule().id("NNG_관형사_일반명사_띄어쓰기")
    .tag(Tag.관형사형전성어미).context()
    .tags({Tag.일반명사, Tag.고유명사}).if_not_spaced()
    .tag(Tag.주격조사).context()
    .msg("명사 '{dform[0]}'batchim(\"을\", \"를\") 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNG_일반부사뒤_띄어쓰기")
    .AND(tag(Tag.일반부사), forms({"계속", "보통", "조금"})).context()
    .tag(Tag.일반명사).if_not_spaced()
    .msg("'{dform[0]}'batchim(\"을\", \"를\") 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNG_수사_시간_띄어쓰기")
    .AND(tags({Tag.관형사, Tag.수사}), forms({"한", "두", "세", "네", "다섯", "여섯", "일곱", "여덟", "아홉", "열", "열두", "한두"}))
    ...
    *rule().id("NNG_수사_날_1_띄어쓰기")
    .AND(tag(Tag.수사), forms({"첫째", "둘째", "셋째", "넷째", "다섯째", "여섯째", "일곱째", "여덟째", "아홉째", "열째"})).context()
    ...
    *rule().id("NNG_OO색_붙여쓰기")
    .AND(tags({Tag.형용사, Tag.형용사규칙활용}), forms({"붉", "푸르", "노랗", "검", "하얗", "희"}))
    ...
    *rule().id("NNG_첫_일반명사_띄어쓰기")
    .tag_form(Tag.관형사, "첫").context()
    .AND(tag(Tag.일반명사), forms({"등장"})).if_not_spaced()
    .msg("'{form[1]}'batchim(\"을\",\"를\") 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNG_관형사 한_앞_붙여쓰기")
    .tag_form(Tag.관형사, "한")
    .AND(tag(Tag.일반명사), forms({"때"})).if_spaced()
    .msg("'한{form[1]}'batchim(\"으로\",\"로\") 붙여 써야 합니다.").build(),
    
    *rule().id("NNG_관형사 한_뒤_띄어쓰기")
    .tag_form(Tag.관형사, "한")
    .AND(tag(Tag.일반명사), forms({"수", "몸", "방울"})).if_not_spaced()
    .msg("'한 {form[1]}'batchim(\"으로\",\"로\") 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNG_이그저_일반명사_띄어쓰기")
    .AND(tag(Tag.관형사), forms({"이", "그", "저"})).context()
    .AND(tag(Tag.일반명사), form("새끼")).if_not_spaced()
    .msg("'{form[0]} {form[1]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),
    ...
    *rule().id("NNG_이그저_의존명사_띄어쓰기")
    .AND(tag(Tag.관형사), forms({"이", "그", "저"}))
    .AND(tag(Tag.의존명사), form("새끼")).if_not_spaced()
    .msg("'{form[0]} {form[1]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),
...
_NNG_SINGLE_WORDS = [
    *rule().id("NNG_SINGLE_관형사형전성어미_띄어쓰기")
    .tags({Tag.관형사형전성어미}).context()
    .AND(tag(Tag.일반명사), forms({"뒤", "정도", "경우"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\", \"를\") 앞 말과 띄어 써야 합니다.").build(),

    *rule().id("NNG_SINGLE_일반/고유명사_띄어쓰기")
    .tags({Tag.일반명사, Tag.고유명사})
    .tag_form(Tag.닫는부호, "'").opt()
    .AND(tag(Tag.일반명사), forms({"더미", "밀착", "능력", "인플레", "초반", "후반", "속", "가능", "무리", "테러", "맵"})).if_not_spaced()
    .msg("'{dform[0]} {form[1]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),

    *rule().id("NNG_SINGLE_일반/의존명사_띄어쓰기")
    .tags({Tag.일반명사, Tag.의존명사}).context()
    .AND(tag(Tag.일반명사), forms({"시절", "비용", "정도", "자체", "투혼"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\", \"를\") 앞 말과 띄어 써야 합니다.").build(),
    
    *rule().id("NNG_SINGLE_뒤에 일반명사_띄어쓰기")
    .AND(tag(Tag.일반명사), forms({"그날"})).context()
    .tag(Tag.일반명사).if_not_spaced()
    .msg("'{form[0]} {dform[0]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),

    *rule().id("NNG_SINGLE_일정_띄어쓰기")
    .tag_form(Tag.일반명사, "일정")
    .tag(Tag.일반명사).if_not_spaced()
    .msg("'일정 {dform[1]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),

    *rule().id("NNG_SINGLE_뭐_띄어쓰기")
    .NOT(tags({Tag.여는부호, Tag.줄임표, Tag.닫는부호, Tag.종결부호})).context()
...
_NNG_NNG = [
    ...
    *NNG_and_NNG("입", "속", SpacingRule.ATTACHED),
...
_VV = [
    *rule().id("VV_일반부사 뒤_띄어쓰기")
    .AND(tag(Tag.일반부사), forms({"미리", "많이", "꽉", "다시", "딱", "빨리", "대충", "깜짝", "같이", "훨씬", "이리", "먼저", "한번"})).context()
    .tags(TagGroup.용언).if_not_spaced()
    .msg('\'merge(({dform[0]}, {dtag[0]}), ("다", "종결어미"))\'를 앞 말과 띄어 써야 합니다.').build(),
    ...
    *rule().id("VV_잘_뒤_띄어쓰기")
    ...
    .AND(tags({Tag.동사, Tag.동사규칙활용, Tag.동사불규칙활용}), NOT(forms({"하", "되", "생기", "빠지"}))).if_not_spaced()
    .msg('\'잘 merge(({dform[0]}, {dtag[0]}), ("다", "종결어미"))\'로 띄어 써야 합니다.').build(),
    ...
    *rule().id("VV_하다_(5)_띄어쓰기")
    .forms({"툭", "탕", "쾅", "쿵"}).context()
    .tag(Tag.닫는부호).opt().context()
    .tag_form(Tag.동사, "하").if_not_spaced()
    .msg("'{form[0]} 하다'로 띄어 써야 합니다.").build(),    

    *rule().id("VV_하다_(6)_띄어쓰기")
    .tags({Tag.일반명사, Tag.고유명사, Tag.대명사, Tag.명사파생접미사}).context()
    .tag_form(Tag.동사, "하").if_not_spaced()
    ...
    *rule().id("VV_못_동사_띄어쓰기")
    .tag_form(Tag.일반부사, "못")
    .AND(tag(Tag.동사), NOT(forms({"하", "되", "쓰"}))).if_not_spaced()
...
_VV_EC_VV = [
    ...
    *VV_EC_VV(("내", "동사"), "어", ("놓", "보조용언"), SpacingRule.ATTACHED),
    ...
    *VV_EC_VV(("먹", "동사"), "고", ("살", "동사"), SpacingRule.ATTACHED, "'생계를 유지하다'의 의미일 경우, '먹고살다'로 붙여 써야 합니다."),
...
_VX = [
    ...
    *rule().id("VX_연결어미_말다_띄어쓰기")
    .AND(tag(Tag.연결어미), NOT(form("자"))).context()
    .tag_form(Tag.보조용언, "말").if_not_spaced()
    .NOT(AND(tags({Tag.연결어미, Tag.종결어미}), form("자"))).context()
    .msg("'말다(마)'를 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("VX_이/그/저렇다 보다_띄어쓰기")
    ...
    .AND(tags({Tag.종결어미, Tag.연결어미}), form("다")).context()
    .tag_form(Tag.보조용언, "보").if_not_spaced()
    .AND(tag(Tag.연결어미), forms({"면", "니까", "니"}))
    ...
    *rule().id("VX_~다시피 하다_띄어쓰기")
    .tag_form(Tag.연결어미, "다시피").context()
    .tag_form(Tag.보조용언, "하").if_not_spaced()
    .msg("'~다시피 하다'로 띄어 써야 합니다.").build(),
    ...
    *rule().id("VX_~려고 하다_띄어쓰기")
    ...
    .tag_form(Tag.보조용언, "하").if_not_spaced()
    .NOT(forms({"다가"})).context()
    .msg("'하다'를 앞 말과 띄어 써야 합니다.").build(),
]

_VA = [
    *rule().id("VA_명사 뒤_띄어쓰기")
    .tag(Tag.일반명사)
    .AND(tag(Tag.형용사), forms({"나쁘", "많"})).if_not_spaced()
...
_VCP = [
    ...
    *rule().id("VCP_의존명사_VCP_다_붙여쓰기")
    .AND(tag(Tag.의존명사), forms({"것", "편", "뿐"})).context()
    .tag(Tag.닫는부호).opt().context()
    .tag(Tag.긍정지정사).if_spaced()
    ...
    *rule().id("VCP_이기도_붙여쓰기")
    ...
    .tag_form(Tag.명사형전성어미, "기")
    .tag_form(Tag.보조사, "도")
    .msg("'이기도'를 앞 말에 붙여 써야 합니다.").build(),
...
_MM = [
    ...
    *rule().id("MM_이그저_뒤_일반명사_띄어쓰기")
    .AND(tag(Tag.관형사), forms({"이", "그", "저"})).context()
    .AND(tag(Tag.일반명사), forms({"사람", "자식", "부분"})).if_not_spaced()
    .msg("'{form[0]} {form[1]}'batchim(\"으로\", \"로\") 띄어 써야 합니다.").build(),
...
_MAG = [
    *rule().id("MAG_명사 뒤_띄어쓰기")
    .tags({Tag.일반명사, Tag.고유명사, Tag.의존명사}).context()
    .AND(tag(Tag.일반부사), forms({"내내", "안"})).if_not_spaced()
    .msg("'{form[0]}'batchim(\"을\", \"를\") 앞 말과 띄어 써야 합니다.").build(),
    ...
    *rule().id("MAG_일반부사뒤_띄어쓰기")
    .AND(tag(Tag.일반부사), forms({"아주"})).context()
    .tag(Tag.일반부사).if_not_spaced()
    .msg('\'{dform[0]}\'batchim("을", "를") 앞 말과 띄어 써야 합니다.').build(),
    ...
    *rule().id("MAG_한번_1_붙여쓰기")
    ...
    .tag_form(Tag.관형사, "한")
    .tag_form(Tag.의존명사, "번").if_spaced()
    .AND(NOT(form("더")), NOT(tag(Tag.긍정지정사))).context()
    .AND(NOT(form("더")), NOT(tag(Tag.긍정지정사))).opt().context()
    .AND(NOT(form("더")), NOT(tag(Tag.긍정지정사))).opt().context()
    ...
    *rule().id("MAG_한번_1_붙여쓰기")
    ...
    .AND(tag(Tag.연결어미), forms({"면", "으면"})).context()
    .msg("'한번 ~하고 나면'의 형태가 쓰였을 경우에는 '한번'을 붙여 써야 합니다.").build(),
    ...
    *rule().id("MAG_뒤의 용언_띄어쓰기")
    .tag_form(Tag.일반부사, "잠깐").context()
    .tags(TagGroup.용언).if_not_spaced()
    .msg("'merge(({dform[0]}, {dtag[0]}), (\"다\", \"종결어미\"))'를 앞 말과 띄어 써야 합니다.").build(),
...
_JX = [
    *rule().id("JX_복합어_보조용언_1_띄어쓰기")
    .AND(tag(Tag.일반명사), longer(2)).context()
    .tag(Tag.닫는부호).opt().context()
    .tag(Tag.동사파생접미사).context().if_not_spaced()
    .tag(Tag.연결어미).context()
    .AND(tag(Tag.보조용언), forms(보조용언_FORMS - {"하"})).if_not_spaced() # ~~해하다 오탐 때문에 '하' 제외
    ...
    *rule().id("JX_복합동사_보조용언_3_띄어쓰기")
    .AND(tag(Tag.동사), forms(복합_3_동사들)).context()
    .tag(Tag.닫는부호).opt().context()
    .tag(Tag.연결어미).context()
    .AND(tag(Tag.보조용언), forms(보조용언_FORMS)).if_not_spaced()
    .msg("'merge(({form[1]}, \"보조용언\"), (\"다\", \"종결어미\"))'를 앞 말과 띄어 써야 합니다.")
    .detail("보조 용언은 앞 말과 붙여 씀이 허용됩니다. 그러나 본용언이 3음절 이상의 복합어인 경우에는 반드시 띄어 써야 합니다.\n예를 들어 '보살펴주다'는 '보살피다'가 '보+살피다'로 이루어진 복합어이므로, 뒤에 오는 보조 용언은 반드시 띄어 써야 합니다.")
    .build(),
    ...
    *rule().id("JX_조차_붙여쓰기")
    .tags(TagGroup.체언 | {Tag.명사파생접미사}).context()
    .tag_form(Tag.보조사, "조차").if_spaced()
    .msg("'조차'를 앞 말에 붙여 써야 합니다.").build(),

    *rule().id("JX_조차_명사파생접미사_붙여쓰기")
    .AND(tag(Tag.명사파생접미사), forms("들")).context()
    .tag_form(Tag.보조사, "조차").if_spaced()
    .msg("'조차'를 앞 말에 붙여 써야 합니다.").build(),
    
    *rule().id("JX_조차_연결어미_붙여쓰기")
    .tag_form(Tag.연결어미, "는지").context()
    .tag_form(Tag.보조사, "조차").if_spaced()
    .msg("'조차'를 앞 말에 붙여 써야 합니다.").build(),
    
    *rule().tags(TagGroup.체언 | {Tag.명사형전성어미}).context()
    .AND(tag(Tag.보조사), forms({"야", "이야"}))
    .form("말").if_spaced()
    .form("로")
    .msg("'~야말로'로 붙여 써야 합니다.").build(),
    
    *rule().tag_form(Tag.일반명사, "시")
    .tag_form(Tag.보조사, "도")
    .tag_form(Tag.일반명사, "때").if_not_spaced()
    .tag_form(Tag.보조사, "도")
    .msg("'시도 때도'로 띄어 써야 합니다.").build(),
    
    *rule().tag_form(Tag.연결어미, "게")
    ...
    *rule().id("JX_까지_붙여쓰기")
    .tags({Tag.일반명사, Tag.고유명사, Tag.명사형전성어미, Tag.대명사, Tag.의존명사, Tag.연결어미}).context()
    .tag_form(Tag.보조사, "까지").if_spaced()
    .msg("'까지'를 앞 말에 붙여 써야 합니다.").build(),

    *rule().id("JX_대로_1_붙여쓰기")
    .tags({Tag.일반명사, Tag.고유명사, Tag.명사형전성어미, Tag.대명사, Tag.의존명사, Tag.알파벳}).context()
    ...
    *rule().id("JX_대로_붙여쓰기")
    .tags({Tag.일반명사, Tag.대명사, Tag.고유명사}).context()
    .tag_form(Tag.보조사, "대로").if_spaced()
    .msg("'대로'를 앞 말에 붙여 써야 합니다.").build(),
]
...
_JKB = [
    ...
    *rule().id("JKB_보다_붙여쓰기")
    .tags({Tag.명사형전성어미, Tag.대명사, Tag.일반명사, Tag.고유명사, Tag.명사파생접미사}).context()
    .tag(Tag.닫는부호).opt()
    .tag_form(Tag.부사격조사, "보다").if_spaced()
    ...
    *rule().id("JKB_용언앞_띄어쓰기")
    .tags({Tag.일반명사, Tag.고유명사}).context()
    .AND(tag(Tag.부사격조사), forms({"와", "으로", "로"})).context()
    .tags(TagGroup.용언).if_not_spaced()
    .msg('\'merge(({dform[0]}, {dtag[0]}), ("다", "연결어미"))\'를 앞 말과 띄어 써야 합니다.').build(),
...
_JKQ = [
    ...
    *rule().id("JKQ_라고_2_붙여쓰기")
    ...
    .msg("'(이)라고'를 앞 말에 붙여 써야 합니다.").build(),
...
_EC = [
    ...
    *rule().id("EC_ㄴ데_명사_붙여쓰기")
    .AND(tag(Tag.일반명사), forms({"출신"})).context()
    .tag(Tag.긍정지정사).context()
    .tag_form(Tag.관형사형전성어미, "ᆫ").context()
    .tag_form(Tag.의존명사, "데").if_spaced()
    .tag_form(Tag.보조사, "도").context()
    ...
    *rule().id("EC_ㄹ뿐더러_붙여쓰기")
    .AND(tag(Tag.관형사형전성어미), batchim("ᆯ")).context()
...
_ETM = [
    *rule()
    .tags({Tag.일반명사, Tag.고유명사, Tag.명사파생접미사, Tag.대명사, Tag.의존명사}).context()
...
_XSA = [
    ...
    *rule().id("XSA_답다_붙여쓰기")
    .NOT(tags({Tag.종결부호, Tag.줄임표})).context()
...
_XSN = [
    ...
    *rule().id("XSN_붙여쓰기")
    .tags({Tag.일반명사, Tag.고유명사, Tag.대명사, Tag.명사형전성어미, Tag.수사}).context()
    ...
    *rule().id("XSN_하다_2_붙여쓰기")
    .NOT(tags({Tag.관형사형전성어미, Tag.일반명사, Tag.관형사, Tag.관형격조사})).context()
    .AND(tag(Tag.일반명사), forms(하다_SHOULD_ATTACHED)).context()
    .tag_form(Tag.동사파생접미사, "하").if_spaced()
    .msg("'하다'를 앞 말에 붙여 써야 합니다.").build(),
    
    *rule().id("XSN_하다_3_붙여쓰기")
    .AND(AND(tag(Tag.일반명사), longer(3)), NOT(forms({"이것저것"})))
    .tag_form(Tag.동사, "하").if_spaced()
    ...
    *rule()
    .AND(tag(Tag.명사파생접미사), NOT(form("들")))
    .tag_form(Tag.동사, "당하").if_spaced()
    ...
    *rule().id("XSN_상_2_붙여쓰기")
    .AND(tag(Tag.일반명사), forms(상_MUST_ATTACHED)).context()
    ...
    *rule().id("XSN_가량_1_붙여쓰기")
    .tags({Tag.관형사, Tag.숫자}).context()
    .forms(날짜_의존명사_FORMS).context()
    .form("가량").if_spaced()
    .msg("'가량'을 앞 말에 붙여 써야 합니다.").build(),
    
    *rule().id("XSN_가량_2_붙여쓰기")
    .OR(tag_form(Tag.기타특수문자, "%"), tag_form(Tag.의존명사, "퍼센트")).context()
    .form("가량").if_spaced()
    .msg("'가량'을 앞 말에 붙여 써야 합니다.").build(),
